...
import collections
import numpy as np
from LDR5_MODEL import LDR5_MODEL
import pandas as pd
...
print(f'ADC Voltage: {str(chan0.voltage)}V, {str(chan1.voltage)}V, {str(chan2.voltage)}V, {str(chan3.voltage)}V, {str(chan4.voltage)}V')
print(f'Raw ADC Values: {chan0.value} {chan1.value} {chan2.value} {chan3.value} {chan4.value}')
...
def getDataFromLDR():
    global LDR0, LDR1, LDR2, LDR3, LDR4
    LDR0 = chan0.value
    LDR1 = chan1.value
    LDR2 = chan2.value
    LDR3 = chan3.value
    channelArray=[LDR0, LDR1, LDR2, LDR3, LDR4]
    return channelArray
...
def predictDirection(aa):
    ...
    Cu = ndimage.center_of_mass(dfObj[0].values)
    Cw = ndimage.center_of_mass(dfObj[1].values)
    Cx = ndimage.center_of_mass(dfObj[2].values)
    Cy = ndimage.center_of_mass(dfObj[3].values)
    Cz = ndimage.center_of_mass(dfObj[4].values)


    pred_data = np.array(list([ Cu[0], Cw[0], Cx[0], Cy[0], Cz[0]])).reshape(1, -1)
